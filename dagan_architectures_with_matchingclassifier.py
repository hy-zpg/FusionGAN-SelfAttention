                            ...
                            if (idx - 3) >= 0:
                                ...
                                for s in range(support_num):
                                    current_sim = tf.expand_dims(
                                        tf.expand_dims(tf.expand_dims(similarities[:, s], -1), -1), -1)
                                    current_encoder = tf.multiply(current_sim, encoder_layers_whole[s][idx - 1])
                                    _, attention_current = self.attention_module(outputs, current_encoder,
                                                                                 name='single_{}'.format(idx),
                                                                                 training=training)
                                    attention.append(attention_current)

                                attention_slim = []
                                encoder_s_attention = []
                                attention_total = tf.reduce_sum(tf.stack(attention, axis=1), axis=1) + 1e-8
                                # attention_total = tf.Print(attention_total, [attention_total], 'attention_total')
                                for s in range(support_num):
                                    current_sim = tf.expand_dims(
                                        tf.expand_dims(tf.expand_dims(similarities[:, s], -1), -1), -1)
                                    current_encoder = tf.multiply(current_sim, encoder_layers_whole[s][idx - 1])
                                    # attention[s] = tf.divide(attention[s], attention_total)
                                    attention_sli = tf.reduce_mean(attention[s], axis=[3])
                                    attention_sli = tf.expand_dims(attention_sli, axis=-1)
                                    attention_sli = tf.image.resize_images(attention_sli,
                                                                           size=[support_input.get_shape()[2],
                                                                                 support_input.get_shape()[2]],
                                                                           method=ResizeMethod.BILINEAR)
                                    ...
                                    # print('0',selected_fusion)
                                    attention_slim.append(selected_fusion)
                                    encoder_s_current = tf.multiply(attention[s], current_encoder)
                                    encoder_s_attention.append(encoder_s_current)
                                ...
                                encoder_s_attention = tf.stack(encoder_s_attention, axis=1)
                                encoder_s_attention = tf.reduce_sum(encoder_s_attention, axis=1)
                                outputs = self.bi_attention_module(outputs, encoder_s_attention,
                                                                   name='decoder_{}'.format(idx),
                                                                   training=training)
                                # outputs = encoder_attention
                                # outputs = tf.concat([outputs,encoder_s_attention],axis=3)
                                current_layers[-1] = outputs
                            '''
